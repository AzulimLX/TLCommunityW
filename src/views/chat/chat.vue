<template>
  ...
  <div class="box">
    <div class="friends_box">
      ...
      <el-scrollbar class="show_friend_box">
        <div v-for="(friendItem, index) in friendItems" :key="friendItem.id">
         <div class="show_friend">
          <!--来点头像-->
           <el-avatar class="friend_Avatar" shape="square" :size="50" :src="friendItem.photo" />
           <!--ID-->
           <div class="friend_name">{{friendItem.username}}</div>
...
import {getAllFriendInfo, getAllFriends} from "@/api/NewApi/Friends";
...
onMounted(async ()=>{
  ...
  scrollToBottom(); //尝试滚到最底部，可惜实现不了

  //直接查询好友
  friend.value = await getAllFriendInfo(nowUser.value)
  //遍历放吧
  friend.value.data.data.forEach( function (item, index){
    friendItems.value.push({id: Date.now(), user: item.user , username: item.username , photo: item.photo})
  })
...
<style src="@/assets/css/chat/chat.css" scoped>
